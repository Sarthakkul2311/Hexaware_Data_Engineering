...
SELECT co.customer_id, b.burger_name, COUNT(co.order_id) AS burger_count
FROM customer_orders co
JOIN burger_names b ON co.burger_id = b.burger_id
GROUP BY co.customer_id, b.burger_name
ORDER BY co.customer_id, b.burger_name;
...
FROM (SELECT order_id, COUNT(burger_id) AS order_count
     FROM customer_orders
     GROUP BY order_id) AS burger_counts;

-- 7. What was the total volume of burgers ordered for each hour of the day?
SELECT DATEPART(hour, order_time) AS order_hour, COUNT(order_id) AS total_burgers_ordered
FROM customer_orders
